...
# y[ss] = NA
# 
# test_that("derivative of Σ is correct without B", {
...
#     tmp_b = y0[i] ^ exp(- (dist_to_s0_from_mesh / λβ) ^ κβ)
#     ll2[i] = mvtnorm::dmvnorm(
#       x = y[good_index, i],
#       mean = y0[i] * α_func(λ, κ, dist_to_s0[good_index]),
#       sigma = Σ_funcs$value(A[good_index, ], A_constr, diag(tmp_b)) + diag(1 / τ, length(good_index)),
#       log = TRUE)
#   }
#   ll3 = ll_conditional(
#     y = y,
...
#       sigma = Σ_funcs$value(A, A_constr, diag(tmp_b)) + diag(1 / τ, nrow(y)),
#       log = TRUE)
#   }
...
# dist_to_s0_from_mesh = lapply(s0_locations, function(i) as.matrix(dist(rbind(loc[i, ], mesh$loc[, -3])))[-1, 1])
# 
# n_repl_per_s0 = 4
# 
# a_funcs = get_a_funcs(θ_a, TRUE)
...
#         tmp_b = y0[[i]][j] ^ exp(- (dist_to_s0_from_mesh[[i]] / λβ) ^ κβ)
#         ll2[length(ll2) + 1] = mvtnorm::dmvnorm(
#           x = y[[i]][, j],
#           mean = y0[[i]][j] * α_func(λ, κ, dist_to_s0[[i]]),
#           sigma = as.matrix(
#             A[[i]] %*% Σ_funcs$value(A_constr = A_constr[[i]], B = diag(tmp_b)) %*% Matrix::t(A[[i]])) +
#             diag(1 / τ, n_loc - 1),
#           log = TRUE)
#       }
#     }
